...
export const PROFILE: IProfile = {
  ...
  location: "경기 성남시",
  company: "㈜우보재난시스템",
  companyPeriod: "2023.10 ~ 현재 (실무경험 1년)",
  summary: [
...
export const PROJECTS: IProject[] = [
    ...
    troubles: [
      {
        ...
        cause:
          "여러 사용자가 동시에 재고 1개 남은 상품 주문 시 음수 재고 발생 가능",
        solution:
          "@Version 기반 Optimistic Lock 적용 + 충돌 시 retry 로직. MockMvc 통합 테스트로 동시 주문 시나리오 검증",
      ...
      },
      {
        ...
        solution:
          "AdminRoute 컴포넌트 신설 (role !== ADMIN → / redirect) + 백엔드 SecurityConfig에 hasRole(ADMIN) 적용해 이중 가드",
        result:
          "admin UI 비인가 노출 차단, 백엔드 데이터도 차단되어 양쪽으로 보호",
      },
      {
        issue: "Mock 데이터로 인한 백엔드 의존성 가림",
        cause:
          "API 실패 시 mock 데이터로 폴백되어 진짜 에러가 가려져 디버깅 어려움",
        solution:
          "일반 화면의 mock 파일·fallback 코드 제거하고 명시적 에러 노출. 단 시연용 admin 대시보드만 폴백 유지",
  ...
  },
  {
    ...
    emoji: "🌊",
    oneliner:
      "현장 장비 데이터를 실시간 시각화하는 관공서 납품 관리자 대시보드",
    description:
...
export const TIMELINE: ITimelineItem[] = [
  {
    period: "2023.10",
    event: "㈜우보재난시스템 입사",
    detail: "시스템사업부 · 웹개발 담당 (사원)",
  },
  {
    period: "2024.05",
    event: "저수지 수위 대시보드 개발",
    detail: "1인 풀스택 (Node.js + Vue 3)",
  },
  {
    period: "2024.10",
    event: "운영지원시스템 (Vue 3) 본격 개발",
    detail: "JWT 인증 · 카카오맵 오버레이 · 점검 이력 CRUD",
  },
  {
    period: "2025.04",
    event: "사원 → 대리 승진",
    detail: "입사 1년 6개월 만에 승진 · 연간 우수사원 · 분기 칭찬사원 선정",
  },
  {
    period: "2025.10",
    event: "Next.js 마이그레이션 착수",
    detail: "Vue 3 → Next.js App Router · Vercel 배포",
  },
  {
    period: "2026.03",
    event: "정보처리산업기사 필기합격",
